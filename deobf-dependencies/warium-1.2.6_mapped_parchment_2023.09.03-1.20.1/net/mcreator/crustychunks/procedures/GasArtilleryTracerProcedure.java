   ...
   public static void execute(LevelAccessor world, double x, double y, double z, Entity immediatesourceentity) {
      if (immediatesourceentity != null) {
         ...
         if (!immediatesourceentity.isUnderWater()) {
            SimpleParticleType var10001 = (SimpleParticleType)CrustyChunksModParticleTypes.GREEN_TRACER.get();
         ...
         if (immediatesourceentity.getPersistentData().getDouble("T") >= 20.0D && world instanceof Level) {
            ...
            if (!_level.isClientSide()) {
               _level.playSound((Player)null, BlockPos.containing(x, y, z), (SoundEvent)ForgeRegistries.SOUND_EVENTS.getValue(new ResourceLocation("entity.firework_rocket.launch")), SoundSource.NEUTRAL, 4.0F, (float)(3.0D - immediatesourceentity.getLookAngle().y * 3.0D));
            } else {
               _level.playLocalSound(x, y, z, (SoundEvent)ForgeRegistries.SOUND_EVENTS.getValue(new ResourceLocation("entity.firework_rocket.launch")), SoundSource.NEUTRAL, 4.0F, (float)(3.0D - immediatesourceentity.getLookAngle().y * 3.0D), false);
            }
         }
