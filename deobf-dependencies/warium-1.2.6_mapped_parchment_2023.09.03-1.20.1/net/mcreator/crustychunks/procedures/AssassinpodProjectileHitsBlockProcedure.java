   ...
   public static void execute(LevelAccessor world, double x, double y, double z, Entity immediatesourceentity) {
      if (immediatesourceentity != null) {
         ...
         CrustyChunksMod.queueServerWork(20, () -> {
            if (world instanceof ServerLevel) {
               ServerLevel _level = (ServerLevel)world;
               Entity entityToSpawn = ((EntityType)CrustyChunksModEntities.ASSASSIN.get()).spawn(_level, BlockPos.containing(x + 0.5D, y + 1.0D, z + 0.5D), MobSpawnType.MOB_SUMMONED);
         ...
         CrustyChunksMod.queueServerWork(20, () -> {
            if (1 == Mth.nextInt(RandomSource.create(), 1, 4) && world instanceof ServerLevel) {
               ServerLevel _level = (ServerLevel)world;
               Entity entityToSpawn = ((EntityType)CrustyChunksModEntities.SCOUT.get()).spawn(_level, BlockPos.containing(x + 0.5D, y + 2.0D, z + 0.5D), MobSpawnType.MOB_SUMMONED);
               if (entityToSpawn != null) {
                  entityToSpawn.setDeltaMovement(0.0D, 0.0D, 0.0D);
               }
            }
