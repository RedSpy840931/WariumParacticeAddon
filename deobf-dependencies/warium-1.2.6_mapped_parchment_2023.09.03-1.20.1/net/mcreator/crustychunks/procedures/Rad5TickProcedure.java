...
public class Rad5TickProcedure {
   public static void execute(LevelAccessor world, double x, double y, double z) {
      if (world instanceof ServerLevel) {
         ...
         _level.sendParticles(ParticleTypes.ASH, x + 0.5D, y + 3.0D, z + 0.5D, 40, 5.0D, 3.0D, 5.0D, 0.0D);
      }
      ...
      while(var9.hasNext()) {
         ...
         if (1000.0D > entityiterator.getPersistentData().getDouble("Radiation")) {
            entityiterator.getPersistentData().putDouble("Radiation", entityiterator.getPersistentData().getDouble("Radiation") + 5.0D);
