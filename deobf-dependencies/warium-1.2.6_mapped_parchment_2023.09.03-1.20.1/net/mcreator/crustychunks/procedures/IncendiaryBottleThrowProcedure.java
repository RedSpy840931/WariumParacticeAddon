...
public class IncendiaryBottleThrowProcedure {
   public static void execute(LevelAccessor world, double x, double y, double z, Entity entity, ItemStack itemstack) {
      if (entity != null) {
         ...
         if (var10000.getItem() == CrustyChunksModItems.INCENDIARY_BOTTLE.get()) {
            Level projectileLevel = entity.level();
            if (!projectileLevel.isClientSide()) {
               Projectile _entityToSpawn = ((<undefinedtype>)(new Object() {
                  public Projectile getArrow(Level level, float damage, int knockback) {
                     AbstractArrow entityToSpawn = new IncendiaryBottleProjectileEntity((EntityType)CrustyChunksModEntities.INCENDIARY_BOTTLE_PROJECTILE.get(), level);
