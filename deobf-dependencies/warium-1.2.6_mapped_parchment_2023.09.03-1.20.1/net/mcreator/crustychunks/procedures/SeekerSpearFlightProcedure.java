...
public class SeekerSpearFlightProcedure {
   public static void execute(LevelAccessor world, double x, double y, double z, Entity immediatesourceentity) {
      if (immediatesourceentity != null) {
         ...
         List<Entity> _entfound = world.getEntitiesOfClass(Entity.class, (new AABB(_center, _center)).inflate(200.0D), (e) -> {
            return true;
         ...
         while(true) {
            ...
            if (Math.sqrt(Math.pow(entityiterator.getY() - y, 2.0D) + Math.pow(entityiterator.getX() - x, 2.0D) + Math.pow(entityiterator.getZ() - z, 2.0D)) - Math.sqrt(Math.pow(entityiterator.getY() - (y + immediatesourceentity.getDeltaMovement().y()), 2.0D) + Math.pow(entityiterator.getX() - (x + immediatesourceentity.getDeltaMovement().x()), 2.0D) + Math.pow(entityiterator.getZ() - (z + immediatesourceentity.getDeltaMovement().z()), 2.0D)) > 1.5D && immediatesourceentity.getPersistentData().getDouble("Time") >= 10.0D) {
