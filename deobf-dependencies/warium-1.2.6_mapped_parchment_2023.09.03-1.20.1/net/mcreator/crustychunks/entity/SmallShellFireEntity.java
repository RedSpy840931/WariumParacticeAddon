...
public class SmallShellFireEntity extends AbstractArrow implements ItemSupplier {
   ...
   public SmallShellFireEntity(SpawnEntity packet, Level world) {
      super((EntityType)CrustyChunksModEntities.SMALL_SHELL_FIRE.get(), world);
   }
   ...
   public void onHitBlock(BlockHitResult blockHitResult) {
      ...
      SmallShellHitProcedure.execute(this.level(), (double)blockHitResult.getBlockPos().getX(), (double)blockHitResult.getBlockPos().getY(), (double)blockHitResult.getBlockPos().getZ(), this);
   }

   public void tick() {
      super.tick();
      AutoCannonShellTracerProcedure.execute(this.level(), this.getX(), this.getY(), this.getZ(), this);
      if (this.inGround) {
         this.discard();
      }
   ...
   public static SmallShellFireEntity shoot(Level world, LivingEntity entity, RandomSource random, float power, double damage, int knockback) {
      SmallShellFireEntity entityarrow = new SmallShellFireEntity((EntityType)CrustyChunksModEntities.SMALL_SHELL_FIRE.get(), entity, world);
   ...
   public static SmallShellFireEntity shoot(LivingEntity entity, LivingEntity target) {
      SmallShellFireEntity entityarrow = new SmallShellFireEntity((EntityType)CrustyChunksModEntities.SMALL_SHELL_FIRE.get(), entity, entity.level());
